...
static int process_command(const char *line, int line_len, write_fn writer, void *wctx, const char *bin_payload, int bin_payload_len) {
    ...
    if (line_len > 5 && strncmp(line, "push ", 5) == 0) {
        ...
        /* find where floats start: first char that is digit, minus, or dot */
        const char *floats_start = data;
        const char *label_end = NULL;
        /* scan for the first float-like character that follows a space */
        for (const char *s = data; s < data + data_len; s++) {
            if ((*s >= '0' && *s <= '9') || *s == '-' || *s == '.') {
                /* check if there's a comma or end soon - confirms it's floats */
                const char *t = s;
                while (t < data + data_len && *t != ' ' && *t != '\n') t++;
                /* if we find a comma in this token, it's the float list */
                for (const char *c = s; c < t; c++) {
                    if (*c == ',') { floats_start = s; label_end = s; goto found_floats; }
                }
                /* single number could be a float or a label - if it's the only token and parses as float, treat as floats */
                if (t >= data + data_len) { floats_start = s; label_end = s; goto found_floats; }
                /* otherwise keep scanning */
            }
        }
        /* no floats found - treat entire thing as error */
        floats_start = data;
        label_end = NULL;
    ...
    found_floats:;
        ...
        int label_len = 0;
        if (label_end && label_end > data) {
            label = data;
            label_len = (int)(label_end - data);
    ...
    if (line_len >= 5 && strncmp(line, "bpush", 5) == 0) {
        ...
        /* label is anything after "bpush " on the header line */
        const char *label = NULL;
        int label_len = 0;
        if (line_len > 6) {
            label = line + 6;
            label_len = line_len - 6;
            while (label_len > 0 && (label[label_len - 1] == ' ' || label[label_len - 1] == '\r')) label_len--;
        }
